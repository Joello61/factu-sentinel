...
final class EligibilityDiagnosticCalculator
{
    ...
    public function calculate(
        ...
        $franchiseRuleVersion = $this->ruleVersionRepository->findActive(RuleId::ELIGIBILITE_FRANCHISE_EN_BASE, $at);

        if (null === $franchiseRuleVersion) {
            throw new RegulatoryRuleVersionNotFoundException(
                sprintf('No active RuleVersion for rule "%s" at %s.', RuleId::ELIGIBILITE_FRANCHISE_EN_BASE, $at->format('Y-m-d')),
            );
        }
