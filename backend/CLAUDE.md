...
Avant toute modification de l'intégration Mistral : consulter la documentation officielle Mistral actuelle, vérifier les modèles disponibles, les endpoints d'API, les limites de taux, les formats de sortie structurée, les recommandations de sécurité, les coûts. Fournisseur français (Mistral) ; l'usage des données à des fins d'entraînement (non par défaut) et l'existence d'un DPA (modèle publié) sont vérifiés au niveau documentaire (`10-security-privacy.md` section 30, Phase 8), mais la localisation effective de traitement et l'exécution contractuelle réelle du DPA pour ce compte restent à confirmer avant mise en production - ne jamais présenter ces points comme définitivement réglés.
...
**Implémentation Phase 8 (état réel)** : `App\AI\Service\MistralProvider` appelle `POST {MISTRAL_BASE_URL}/v1/chat/completions` sans jamais transmettre de clé `tools` (aucune surface d'appel d'outil exposée au modèle, `10-security-privacy.md` section 31). Modèle retenu : `mistral-small-latest` (alias documenté par Mistral, jamais un identifiant daté figé) - comparé sommairement au niveau "medium/large" avant de choisir : fenêtre de contexte très largement suffisante à tous les niveaux pour l'usage visé (reformulation courte / question-réponse ancrée sur `02-regulatory-study.md`, jamais un raisonnement complexe), donc le plus petit niveau adéquat retenu plutôt que le plus gros par défaut (section 15 de `06-technical-architecture.md`, limitation des coûts). Les deux endpoints (`POST /compliance-findings/{id}/explanations`, `POST /assistant/questions`) sont **synchrones** : un seul appel HTTP borné dans le cycle requête/réponse, jamais de Messenger/worker - décision documentée dans `08-api-specification.md` section 35, corrigeant son `Async: Oui` initial (même précédent que la Phase 5 sur les analyses de conformité). Vérification email (`User::isEmailVerified()`, déjà existant mais non appliqué ailleurs avant cette phase) appliquée uniquement sur ces deux endpoints, jamais rétroactivement sur Document/Compliance (écart préexistant laissé en dette Phase 10). Le texte intégral de `02-regulatory-study.md` (ancrage de `/assistant/questions`) est embarqué comme copie build-time synchronisée dans `backend/src/AI/Resources/`, jamais lu depuis `docs/` au runtime (contexte de build Docker du service `backend` limité à `./backend`) - voir `backend/bin/sync-regulatory-context.php` et le test de synchronisation associé avant toute modification de ce document réglementaire.
