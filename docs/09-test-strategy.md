...
| Section 8, définition facture électronique       | `format-facture-electronique`                                       | REG-006                                   | Finding explicite sur PDF simple                                                                                          | Règle **seedée en Phase 5** (`confidence_level = MOYEN`, à revérifier sur les sources DGFiP/AIFE avant activation, `02-regulatory-study.md` section 9) mais son applicabilité (`sources: DOCUMENT_IMPORTE`) ne peut être satisfaite avant la Phase 7 (`Document` n'existe pas) : produit systématiquement `NON_APPLICABLE` en Phase 5 (20/08/2026, `ComplianceEngineTest::testFormatRuleIsAlwaysNonApplicableForManualEntry`). REG-006 lui-même (finding réel sur PDF simple) reste **non implémenté avant Phase 7** |
