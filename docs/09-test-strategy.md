...
| Section 10, mention SIREN client                 | `mention-siren-client`                                              | REG-002, REG-003, REG-004, REG-007        | `NON_CONFORME` si absent (PRO FR, non exonéré) ; `NON_APPLICABLE` sinon                                                   | Implémentée et testée en Phase 5 (20/08/2026, `ComplianceEngineTest::testReg004MissingSirenIsNonConforme`/`testReg002ParticulierCustomerIsNonApplicable`/`testReg003ProfessionnelEtrangerIsNonApplicable`/`testReg007VatExemptOperationIsNonApplicable`, `RunComplianceAnalysisControllerTest::testMissingSirenProducesNonConformeWithCorrectionAction`) ; à renseigner de nouveau à chaque exécution suivante de la suite de régression (section 13)                                                                |
| Section 10, mention catégorie de l'opération     | `mention-categorie-operation`                                       | REG-002, REG-003, REG-007 (applicabilité) | `CONFORME` si `operation_type` renseigné (toujours vrai en saisie manuelle) ; `NON_APPLICABLE` hors périmètre e-invoicing | Implémentée et testée en Phase 5 (20/08/2026, `ComplianceEngineTest`) ; à renseigner de nouveau à chaque exécution suivante de la suite de régression (section 13)                                                                                                                                                                                                                                                                                                                                                   |
| Section 8, définition facture électronique       | `format-facture-electronique`                                       | REG-006                                   | Finding explicite sur PDF simple                                                                                          | Règle seedée en Phase 5 (`confidence_level = MOYEN`), applicabilité (`sources: DOCUMENT_IMPORTE`) non satisfaite avant Phase 7 (`NON_APPLICABLE` systématique, `ComplianceEngineTest::testFormatRuleIsAlwaysNonApplicableForManualEntry`). **Mise à jour Phase 7** : `RuleVersion` v2 publiée, `confidence_level` MOYEN → ÉLEVÉ (revérifiée sur les sources DGFiP/AIFE avant activation, `02-regulatory-study.md` section 9), REG-006 implémenté et testé (`DocumentFormatRuleCheckerTest`, PDF_SIMPLE → `NON_CONFORME`). **Vérifié de bout en bout en Phase 11** (20/08/2026, `frontend/e2e/specs/e2e-004-document-format.spec.ts`, E2E-004) : import réel d'un PDF simple via l'UI, traitement asynchrone, analyse, finding NON_CONFORME affiché avec message explicite - seul le niveau Unit/Integration l'avait couvert jusque-là |
| Section 26, niveaux de confiance                 | (mécanisme générique de l'évaluateur, pas une règle seedée précise) | REG-010                                   | `INCERTAIN_REGLEMENTAIRE` si `confidence_level != ELEVE`                                                                  | Implémentée et testée en Phase 5 au niveau Unit (20/08/2026, `ComplianceRuleEvaluatorTest::testLowConfidenceOverridesCheckResult`, BR-COMPLIANCE-004) ; aucune règle seedée à ce jour n'atteint ce chemin en conditions réelles (`format-facture-electronique` reste bloquée à `NON_APPLICABLE` avant sa confiance, voir ligne ci-dessus)                                                                                                                                                                            |
...
| ----------------------------------------------------- | ---------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ | ---------------------------------- |
| Règle réglementaire mal interprétée                   | Critique - décision de conformité incorrecte transmise à l'utilisateur                   | Moyenne, malgré la rigueur de `02-regulatory-study.md`                 | Traçabilité systématique (section 52), validation humaine (section 56)                     | REG-_ (section 9), TC-COMPLIANCE-_ |
