...
- Idempotence sur les opérations concernées (section 20 de l'API) : clés d'idempotence conservées **24h par défaut** (Redis, clé `idempotency:{tenant}:{key}`, `TTL = 24h`, `08-api-specification.md`). Test dédié : rejouer une même requête A (même `Idempotency-Key`) plusieurs fois de suite doit produire **une seule opération métier effective** (par exemple, une seule `ComplianceAnalysis` créée), la même réponse étant renvoyée à chaque rejeu tant que la fenêtre de 24h n'est pas expirée ; passé ce délai, une requête portant la même clé doit être traitée comme une nouvelle opération distincte.
