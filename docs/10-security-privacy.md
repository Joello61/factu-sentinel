...
**Fournisseur retenu : Mistral** (décision produit, `06-technical-architecture.md` ADR-007) - société française, ce qui change favorablement le contexte de cette évaluation par rapport à la version initiale de ce document (qui envisageait un fournisseur non déterminé, potentiellement hors UE). Les points suivants doivent néanmoins être **vérifiés contractuellement avant mise en production**, la nationalité du fournisseur ne dispensant pas de cette vérification :

- Localisation effective du traitement des données par l'API Mistral utilisée (le siège social en France ne garantit pas à lui seul que chaque requête API est traitée sur un serveur situé en France ou dans l'UE - à confirmer dans la documentation contractuelle ou technique de Mistral, section 45).
- Utilisation ou non des données transmises via l'API à des fins d'entraînement du modèle - **à vérifier avant mise en production** ; les offres API professionnelles excluent généralement cet usage par défaut, mais cette garantie doit être confirmée pour l'offre précisément retenue plutôt que supposée.
- Durée de rétention des données par Mistral pour les requêtes API.
- Existence d'un accord de sous-traitance (DPA) avec Mistral couvrant le traitement de données personnelles transmises via l'AI Gateway.
- Sous-traitants ultérieurs de Mistral, le cas échéant (chaîne de sous-traitance).
...
| Fournisseur email                            | Adresse email, contenu du message                       | Notifications, récupération de compte | Moyen                                                                  | Requis                                           | Non tranché                                                                                                                                                                | Possible selon le fournisseur qui sera retenu                                                                              |
...
**Fournisseurs restant à choisir** : email et vérification d'entreprise. Pour ces deux catégories, ce tableau pose le cadre d'évaluation à appliquer avant intégration, pas une évaluation d'un fournisseur précis.
...
| Fournisseur(s) externes encore non choisis (email, vérification d'entreprise) et leur conformité (sections 44-45)                                                                   | Conditionne l'évaluation de transfert international et de sous-traitance pour ces deux catégories ; aucun fournisseur n'est nommé par les décisions produit actuelles                   | `04-product-requirements.md` section 32, décision à prendre avant mise en production |
