...
- **AI Gateway** ne peut être appelé qu'*après* que le Compliance Engine ait produit un résultat ; il n'a pas d'accès direct aux données de Invoices, Company ou Customers en dehors du contexte explicitement transmis par le Compliance Engine (voir section 15, minimisation des données envoyées à l'IA).
...
Exigence fondamentale directement héritée de `04-product-requirements.md` (section 24) et `05-user-stories.md` (US-HISTORY-001) : pouvoir répondre à *« quelle version des règles a été utilisée pour analyser cette facture ? »*, y compris longtemps après une mise à jour des règles.
...
Ce principe d'immutabilité est ce qui garantit la propriété énoncée dans les règles absolues de ce document (règle 19) : *les résultats historiques doivent rester explicables*, indépendamment des évolutions ultérieures du moteur de règles.
...
Le système doit pouvoir répondre à *« qui a fait quoi, quand, et dans quel contexte ? »*, exigence directement héritée de `04-product-requirements.md` (section 24).
