...
| DL-016 | Fournisseur email retenu : **Brevo**, en SMTP relay (`MAILER_DSN`), jamais l'API dédiée - `backend/.env` ne définissait qu'un défaut de dev `null://null`, jamais surchargé en production (aucun email réellement envoyé jusqu'ici, constaté le 27/08/2026 en préparant la vérification du critère de clôture de la Phase 18) | Résout la question ouverte « fournisseur email non choisi » (`10-security-privacy.md` section 44, 69) - société française, DPA publié, palier gratuit suffisant au volume actuel, `symfony/mailer` déjà installé donc aucune dépendance Composer supplémentaire (SMTP générique nativement supporté) | Met à jour `10-security-privacy.md` section 44/45/69 (fournisseur email désormais tranché, localisation/DPA à confirmer contractuellement, même réserve que Mistral) ; `docker-compose.prod.yml` (`MAILER_DSN` ajouté à l'environnement de `backend` uniquement, jamais `worker`) et `.env.prod.example` |
