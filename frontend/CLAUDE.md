...
**Écart de documentation corrigé (constaté en Phase 16)** : ce paragraphe décrivait encore le projet comme un squelette Next.js non développé au-delà de `app/page.tsx`/`app/layout.tsx` - obsolète depuis plusieurs phases (voir `../docs/12-roadmap.md` pour l'état réel, phase par phase ; le frontend couvre déjà l'ensemble des Epics listés section 3 de ce fichier, plus la surface Platform Admin des Phases 15-16). Versions réellement installées (vérifier à nouveau si ce fichier vieillit) : **Next.js 16.3.1**, **React 19.2.8**, **TypeScript ^5**, **Tailwind CSS v4** (via `@tailwindcss/postcss`). Bibliothèques UI complémentaires actées par `11-frontend-design-system.md` (section 57, 70) : **Lucide React** (icônes) est installé ; **Radix UI** (primitives accessibles) ne l'est pas encore à ce jour - vérifier `package.json` avant de supposer l'un ou l'autre état, jamais ce paragraphe seul, et vérifier la compatibilité React 19/Next 16 avant tout ajout.
...
**Limitation historique résolue (Phase 17, vérifié le 26/08/2026)** : `next build` échouait sur Next.js 16.0.x-16.3.1 lors de la pré-génération de `/_global-error` (bug amont vercel/next.js#86178, #84994, #95741), masqué par une seconde erreur indépendante (TypeScript, voir ci-dessous) qui faisait échouer le build encore plus tôt et rendait le premier bug invisible en pratique. Les deux sont résolus sur Next.js 16.3.3 : `next build` complet, image Docker de production (`frontend/Dockerfile`, cible `prod`) construite et conteneur démarré/testé avec succès. La seconde erreur, `app/(app)/company/CompanyForm.test.tsx` (`Property 'fiscal_context' does not exist on type 'never'`), était une inférence de contrôle de flux TypeScript sur un `let x: T | null = null` réassigné dans une closure - corrigée en remplaçant l'initialisation par `| undefined` non initialisé, comportement identique à l'exécution. `.github/workflows/lint.yml` n'exécutait jamais `npm run build` (seulement `lint`/`test`), ce qui a permis à cette régression TypeScript de rester invisible en CI - un job CI `next build` est désormais requis (voir Phase 17, `docs/12-roadmap.md`).
...
- Palette : `Primary #00695C`, `Success #1B5E20`, `Warning #984501`, `Error #B71C1C`, `Info #01579B` (`frontend/app/globals.css`, `--color-*`) - valeurs assombries en Phase 11 suite à un scan `@axe-core/playwright` automatisé qui révélait un contraste WCAG 2.2 AA insuffisant sur les deux contextes d'usage réels (`border-X bg-X/10 text-X`, `bg-X/15 text-X`) avec les teintes d'origine ; contraste revérifié formellement à cette occasion, pas à revalider par réflexe. Les valeurs elles-mêmes ne sont pas à renégocier sans décision produit explicite. Les couleurs de conformité (section 5 du design system) restent un système **distinct** des couleurs sémantiques génériques, même si des teintes de base sont proches.
- Typographie : **Inter**, fallback `system-ui, sans-serif` ; chiffres tabulaires pour tout montant/TVA.
- UI : **Tailwind CSS v4** + composants internes + **Lucide React** (icônes, installé) - **Radix UI** (primitives accessibles) reste acté par le design system (section 57, 70) mais n'a pas encore été installé au moment où ce fichier a été vérifié (vérifier `package.json` avant de supposer l'un ou l'autre état) ; pas de bibliothèque de composants imposant son propre design visuel.
...
- Graphiques : **aucun au dashboard utilisateur final** (toujours non MVP pour cette surface, DL-008/DL-015 non rouverts) - **Recharts** introduit en Phase 16, exclusivement dans la surface d'administration plateforme (`docs/11-frontend-design-system.md`, section 48).
