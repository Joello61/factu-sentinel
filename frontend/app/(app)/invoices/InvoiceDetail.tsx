...
export function InvoiceDetail({ invoiceId }: { invoiceId: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setState({ status: "error", message: "Impossible de charger cette facture pour le moment." });
      }
    })();
  ...
  if (state.status === "not-found") {
    return <p className="text-sm text-muted-foreground">Cette facture n&apos;existe pas ou n&apos;est plus disponible.</p>;
  }
    ...
    <div className="flex max-w-3xl flex-col gap-6">
      <div>
        ...
        <div className="mt-2 flex items-center gap-3">
          <h1 className="text-2xl font-semibold text-foreground">{invoice.invoice_number ?? "Facture sans numéro"}</h1>
      ...
      <dl className="grid grid-cols-1 gap-4 rounded-md border border-border p-4 sm:grid-cols-2">
        <div>
          ...
          <dd className="text-sm text-foreground">
            {customer ? `${customer.name} (${CUSTOMER_TYPE_LABELS[customer.customer_type]})` : "—"}
          </dd>
        </div>
        <div>
          <dt className="text-xs font-medium text-muted-foreground">Date d&apos;émission</dt>
          <dd className="text-sm text-foreground">{formatBusinessDate(invoice.issue_date)}</dd>
        </div>
        <div>
          <dt className="text-xs font-medium text-muted-foreground">Nature de l&apos;opération</dt>
          <dd className="text-sm text-foreground">{OPERATION_TYPE_LABELS[invoice.operation_type]}</dd>
        </div>
        <div>
          <dt className="text-xs font-medium text-muted-foreground">Origine</dt>
          <dd className="text-sm text-foreground">{invoice.source === "SAISIE_MANUELLE" ? "Saisie manuelle" : "Document importé"}</dd>
        </div>
      </dl>
            ...
            <tbody>
              {invoice.lines.map((line) => (
                <tr key={line.id} className="border-t border-border">
                  ...
                  <td className="px-4 py-2 text-right tabular-nums text-foreground">{line.quantity}</td>
      ...
      <div className="rounded-md border border-primary bg-primary/5 px-4 py-3">
        <p className="text-sm font-medium text-foreground">
          Total HT : <span className="tabular-nums">{formatAmount(invoice.total_amount_ht, invoice.currency)}</span>
        </p>
        <p className="text-sm font-medium text-foreground">
          Total TTC : <span className="tabular-nums">{formatAmount(invoice.total_amount_ttc, invoice.currency)}</span>
        </p>
      </div>
    </div>
  );
